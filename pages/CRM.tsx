...
import { Users, Building2, User, Phone, Mail, Star, Clock, ShoppingBag, MessageCircle, Search, Plus, Wallet, ArrowRight, Pencil, X, ArrowLeft } from 'lucide-react';
...
const CRMPage: React.FC = () => {
  ...
  return (
    <div className="flex flex-col lg:flex-row h-[calc(100vh-8rem)] lg:h-[calc(100vh-2rem)] gap-4 lg:gap-6">
      ...
      {/* LEFT COLUMN: LIST - Hidden on mobile when item selected */}
      <div className={`
          w-full lg:w-1/3 bg-white rounded-2xl shadow-sm border border-slate-200 flex-col overflow-hidden
      ...
      {/* RIGHT COLUMN: DETAILS - Hidden on mobile when no selection */}
      <div className={`
          flex-1 bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex-col
      ...
      `}>
          {selectedCustomer ? (
              <div className="flex flex-col h-full">
                  ...
                  <div className="h-32 bg-gradient-to-r from-earth-600 to-earth-800 p-6 flex justify-between items-start text-white relative">
                      ...
                      <button 
                          ...
                          className="absolute top-4 left-4 p-2 bg-black/20 text-white rounded-full lg:hidden hover:bg-black/30 backdrop-blur-sm z-10"
                      >
                          <ArrowLeft size={20} />
                      </button>

                      <div className="mt-auto pl-8 lg:pl-0">
                          <div className="flex items-center gap-2 mb-1">
                              <h1 className="text-2xl md:text-3xl font-bold truncate max-w-[200px] md:max-w-none">{selectedCustomer.name}</h1>
                              {stats?.totalSpent > 1000 && <Star className="text-yellow-400 fill-yellow-400 flex-shrink-0" size={24} />}
                          </div>
                          <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-4 text-xs md:text-sm opacity-90">
                  ...
                  {/* Stats Grid */}
                  <div className="p-4 md:p-6 grid grid-cols-2 md:grid-cols-4 gap-4 border-b border-slate-100">
                      <div className="p-4 bg-slate-50 rounded-xl">
                          ...
                          <p className="text-xl md:text-2xl font-black text-slate-800">RM {stats?.totalSpent.toFixed(2) || '0.00'}</p>
                      </div>
                      <div className="p-4 bg-slate-50 rounded-xl">
                          ...
                          <p className="text-xl md:text-2xl font-black text-slate-800">{stats?.orderCount || 0}</p>
                      </div>
                      <div className="p-4 bg-slate-50 rounded-xl">
                          <p className="text-xs font-bold text-slate-400 uppercase mb-1 flex items-center"><Star size={12} className="mr-1"/> Favorite</p>
                          <p className="text-sm md:text-lg font-bold text-slate-800 truncate" title={stats?.favoriteProduct}>{stats?.favoriteProduct || '-'}</p>
                      </div>
                      <div className="p-4 bg-slate-50 rounded-xl">
                          <p className="text-xs font-bold text-slate-400 uppercase mb-1 flex items-center"><Clock size={12} className="mr-1"/> Last Order</p>
                          <p className="text-sm md:text-lg font-bold text-slate-800">{stats?.lastOrderDate ? new Date(stats.lastOrderDate).toLocaleDateString() : 'Never'}</p>
                      </div>
                  </div>

                  {/* Actions & Notes - SCROLLABLE */}
                  <div className="p-4 md:p-6 flex-1 bg-slate-50/50 overflow-y-auto custom-scrollbar">
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-slate-400 p-8 text-center">
                  ...
                  <p className="text-lg">Select a customer from the list to view their full profile and history</p>
              </div>
          )}
      </div>
