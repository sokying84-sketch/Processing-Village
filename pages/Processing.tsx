...
// ... (keep all hooks and helper components like useRecipes, useNow, PersistentBatchCard, RecipeSelectorModal same as before) ...
const useRecipes = () => {
               ...
               <div className="space-y-3 pt-2 animate-in slide-in-from-bottom-2 bg-slate-50 p-3 rounded-lg border border-slate-200">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                      <div>
                          ...
                          <input 
                            type="number" 
                            className="w-full p-2 text-sm border border-nature-200 bg-nature-50 rounded focus:ring-1 focus:ring-nature-500"
                            placeholder="0.0"
                            value={qcGoodWeight}
...
const ProcessingPage: React.FC = () => {
  ...
  const handleSaveRecipe = async (e: React.FormEvent) => {
    ...
    const recipeData = {
        ...
        ingredients: editingRecipe.ingredients || '', // Added ingredients
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
             ...
             <div className="flex items-center space-x-3 w-full md:w-auto">
                ...
                <button 
                   ...
                   className="flex-1 md:flex-none px-5 py-2.5 bg-nature-600 text-white rounded-lg font-bold shadow-sm hover:bg-nature-700 flex items-center justify-center transition-all"
                >
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
             <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
                 <div className="flex justify-between items-center mb-6">
                    <h3 className="text-xl font-bold text-slate-900">{editingRecipe.id ? 'Edit Recipe' : 'New Recipe'}</h3>
                    <button onClick={() => setShowRecipeModal(false)} className="p-2 hover:bg-slate-100 rounded-full text-slate-400"><X size={20}/></button>
                 </div>
                 
                 <form onSubmit={handleSaveRecipe} className="space-y-5">
                     <div>
                        <label className="block text-xs font-bold text-slate-500 mb-1.5 uppercase tracking-wide">Recipe Name</label>
                        <input placeholder="e.g. Crispy Oyster Mushrooms" className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-nature-500 focus:bg-white transition-all outline-none" value={editingRecipe.name || ''} onChange={e => setEditingRecipe({...editingRecipe, name: e.target.value})} required />
                     </div>
                     
                     <div className="grid grid-cols-3 gap-4">
                        <div>
                           <label className="block text-xs font-bold text-slate-500 mb-1.5 uppercase tracking-wide flex items-center"><Scale size={12} className="mr-1"/> Base Wt (kg)</label>
                           <input type="number" step="0.1" placeholder="0.5" className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium focus:bg-white outline-none" value={editingRecipe.baseWeightKg || 0.5} onChange={e => setEditingRecipe({...editingRecipe, baseWeightKg: parseFloat(e.target.value)})} />
                        </div>
                        <div>
                           <label className="block text-xs font-bold text-slate-500 mb-1.5 uppercase tracking-wide flex items-center"><Clock size={12} className="mr-1"/> Time (min)</label>
                           <input type="number" placeholder="10" className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium focus:bg-white outline-none" value={editingRecipe.cookTimeMinutes || ''} onChange={e => setEditingRecipe({...editingRecipe, cookTimeMinutes: parseFloat(e.target.value)})} />
                        </div>
                        <div>
                           <label className="block text-xs font-bold text-slate-500 mb-1.5 uppercase tracking-wide flex items-center"><ThermometerSun size={12} className="mr-1"/> Temp (°C)</label>
                           <input type="number" placeholder="160" className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium focus:bg-white outline-none" value={editingRecipe.temperature || ''} onChange={e => setEditingRecipe({...editingRecipe, temperature: parseFloat(e.target.value)})} />
                        </div>
                     </div>

                     <div className="grid grid-cols-1 gap-4">
                        <div>
                           <label className="block text-xs font-bold text-slate-500 mb-1.5 uppercase tracking-wide flex items-center"><List size={14} className="mr-1"/> Ingredients (Shop Display)</label>
                           <textarea 
                              placeholder="List ingredients here (e.g. Mushrooms, Sea Salt, Olive Oil)..." 
                              className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl h-24 text-sm focus:ring-2 focus:ring-nature-500 focus:bg-white outline-none resize-none"
                        ...
                        <div>
                           <label className="block text-xs font-bold text-slate-500 mb-1.5 uppercase tracking-wide flex items-center"><FileText size={14} className="mr-1"/> Description / Notes</label>
                           <textarea 
                              placeholder="Process notes or marketing description..." 
                              className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl h-24 text-sm focus:ring-2 focus:ring-nature-500 focus:bg-white outline-none resize-none"
                     ...
                     <label className="flex flex-col items-center justify-center w-full p-4 border-2 border-dashed border-slate-300 rounded-xl cursor-pointer hover:border-nature-500 hover:bg-nature-50 text-slate-500 relative overflow-hidden h-40 transition-all group">
                        {editingRecipe.imageUrl ? (
                           <div className="absolute inset-0 w-full h-full">
                               <img src={editingRecipe.imageUrl} className="w-full h-full object-cover opacity-50 group-hover:opacity-40 transition-opacity" />
                               <div className="absolute inset-0 flex items-center justify-center">
                                   <div className="bg-white/90 backdrop-blur-sm px-4 py-2 rounded-full shadow-sm text-xs font-bold text-slate-700 flex items-center">
                                       <Pencil size={12} className="mr-2"/> Change Photo
                                   </div>
                               </div>
                           </div>
                        ) : (
                           <>
                              <div className="bg-slate-100 p-3 rounded-full mb-2 group-hover:scale-110 transition-transform"><Upload size={24} className="text-slate-400" /></div>
                     ...
                     <div className="flex space-x-3 pt-2">
                        <button type="button" onClick={() => setShowRecipeModal(false)} className="flex-1 py-3 text-slate-500 hover:bg-slate-50 rounded-xl font-bold transition-colors">Cancel</button>
                        <button type="submit" className="flex-1 py-3 bg-nature-600 text-white rounded-xl font-bold hover:bg-nature-700 shadow-lg hover:shadow-xl transition-all">Save Recipe</button>
                     </div>
                 </form>
             </div>
          </div>
